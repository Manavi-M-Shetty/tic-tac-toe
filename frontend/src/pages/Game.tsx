// Game.tsx
// -------------------------------------------------------
// This component handles the entire Tic-Tac-Toe gameplay.
// It connects to the backend using Socket.io, joins a game
// room, listens for live game updates, shows the board,
// lets the user make moves, and displays win/lose/draw
// animations. It also manages the player's role (X/O) and
...
// Backend base URL (WebSocket)
const API_BASE = import.meta.env.VITE_API_URL?.replace('/api','') || 'http://localhost:4000';

// Helpers to read login info
...
export default function Game(){
  ...
  const [game, setGame] = useState<any>(null); // Stores full game state
  const [socket, setSocket] = useState<any>(null); // Socket connection
  const [showWinAnimation, setShowWinAnimation] = useState(false); // Win/lose/draw animation
  const userId = getUserId(); // Logged-in user ID

  // Exit button → disconnect socket + go back to lobby
  function exitGame() {
    socket?.disconnect();
    nav('/lobby');
  }
  ...
  // -------------------------------------------------------
  // Setup: Connect socket, join game room, and listen for updates
  // -------------------------------------------------------
  useEffect(() => {
    const s = io(API_BASE);
    setSocket(s);
    ...
    s.on('game-state', (state: any) => {
      // Decide if user is X, O, or spectator
      const userRole =
        state.player_x === userId ? 'X' :
        state.player_o === userId ? 'O' :
        null;

      const newGame = { ...state, userRole };
      setGame(newGame);

      // Show animation on game finish
    ...
    // If move is invalid, backend sends an error
  ...
  // -------------------------------------------------------
  // Make a move when user clicks a cell
  // -------------------------------------------------------
  ...
  // -------------------------------------------------------
  // Loading screen until game state is received
  // -------------------------------------------------------
  if (!game) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 sm:p-6 lg:p-8 relative overflow-hidden">
      ...
      {/* Win/Lose/Draw animated overlay */}
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 font-mono">
                Code: {game.game_code || game.id?.slice(0,8)}
              </p>
            </div>
          </div>
